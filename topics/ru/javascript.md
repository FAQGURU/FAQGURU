## JavaScript
  ...
  [Как бы вы использовали закрытие для создания приватного счетчика?](#как-бы-вы-использовали-закрытие-для-создания-приватного-счетчика)
...
[[↑] Вернуться к началу](#JavaScript)
### Как бы вы использовали закрытие для создания приватного счетчика?

Вы можете создать функцию во внешней функции (закрытие), которая позволяет вам обновлять приватную переменную, но переменная не будет доступна вне функции без использования вспомогательной функции.

```js
function counter() {
  var _counter = 0;
  // возвращаем объект с несколькими функциями, которые позволяют
  // изменить приватную переменную _counter
  return {
    add: function(increment) { _counter += increment; },
    retrieve: function() { return 'Текущее значение счетчика: ' + _counter; }
  }
}

// ошибка, если мы попытаемся получить доступ к приватной переменной, как показано ниже
// _counter;

// использование нашей функции счетчика
var c = counter();
c.add(5); 
c.add(9); 

// теперь мы можем получить доступ к приватной переменной следующим образом
c.retrieve(); // => Текущее значение счетчика: 14
```
